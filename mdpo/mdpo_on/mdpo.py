...
class MDPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        #irs: Optional[IntrinsicRewardModule] = None,
    ):
        super().__init__(
            policy,
            env,
    ...
    def log_q(self, x, q):
        safe_x = th.clamp(x, 1e-6)

        #log_q_x = th.cond(th.equal(q,1.), true_fn=lambda: th.log(safe_x), false_fn=lambda: (th.pow(safe_x,1-q)-1)/(1-q))
        if q == 1.0:
            log_q_x = th.log(safe_x)
        else:
            log_q_x = (th.pow(safe_x, 1 - q) - 1) / (1 - q)
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(batch_size=None):
            ...
            with th.no_grad():
                ...
                closed_distribution = self.policy.get_distribution(rollout_data.observations)


            distribution = self.policy.get_distribution(rollout_data.observations)
            log_prob = distribution.log_prob(actions)
            log_prob_old = closed_distribution.log_prob(actions)
            ...
            if self.tsallis_q == 1.0:
                ...
                kl_div = th.mean(self.log_q(th.exp(log_prob), tsallis_q) - self.log_q(th.exp(log_prob_old), tsallis_q))
            ...
            hessian_vector_product_fn = partial(self.hessian_vector_product, actor_params, grad_kl) #, self.sgd_steps)
            ...
            # Maximal step length _R -> Deep reinforcement learning, P174, Eq(5.34)
            ...
            with th.no_grad():
                ...
                for _ in range(self.sgd_steps):
                    ...
                    entbonus = self.ent_coef * entropy_loss
                    entropy_losses.append(entropy_loss.item())
                    
                    if self.method == "multistep-SGD":
                        policy_loss = (ratio * advantages).mean() - kl_div / self.learning_rate
                    elif self.method == "closedreverse-KL":    
                        policy_loss = (th.exp(advantages).mean() * distribution.log_prob(actions))
                    else:
                        policygain = (th.exp(advantages).mean() * th.log(closed_distribution.mean())) 
                        policy_loss = (ratio * advantages).mean() - (self.learning_rate * ratio * distribution.log_prob(actions)).mean()
        ...
        for _ in range(self.n_critic_updates):
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                else: # Clip the difference between old and new value
                    values_pred = rollout_data.old_values + th.clamp(
                        values_pred.flatten() - rollout_data.old_values, -clip_range_vf, clip_range_vf)  
