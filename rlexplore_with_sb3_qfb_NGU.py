...
def main():
    ...
    save_dir = f'./logs_f/QEB_envs/E_MDPO/NGU/{random_seed}/'
    ...
    callback_chkpt = CheckpointCallback(save_freq=1000, save_path=save_dir, name_prefix='NGU-MDPO')
    eval_callback = EvaluationAndCheckpointCallback(eval_env, save_dir=save_dir)
    # ===================== build the reward ===================== #
    irs = NGU(envs=envs, device = device, batch_size=64, latent_dim=30, lr=1e-3, k=10, beta=0.3) 
    # ===================== build the reward ===================== #
    ...
    model.save(f'./logs_f/models/MDPO_E_NGU_{random_seed}_{env_id}')
    end = time.localtime()

    f = open("outputs.txt", "a")
    f.write(f"model MDP_E_NGU with seed = {random_seed} on {env_id} finished in start time_{start} and end time_{end} mean_reward {mean_reward} std_reward {std_reward} \n")
    f.close()
