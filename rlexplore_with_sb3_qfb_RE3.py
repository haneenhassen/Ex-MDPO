...
from rllte.xplore.reward import NGU, RE3
...
def main():
    ...
    save_dir = f'./logs_f/QEB_envs/E_MDPO/RE3/{random_seed}/'
    ...
    callback_chkpt = CheckpointCallback(save_freq=1000, save_path=save_dir, name_prefix='RE3-MDPO')
    eval_callback = EvaluationAndCheckpointCallback(eval_env, save_dir=save_dir)

    # ===================== build the reward ===================== #
    irs = RE3(envs=envs, device=device, k=3, latent_dim=128, beta=0.005, kappa=0.00001, storage_size=1000)
    # ===================== build the reward ===================== #
    ...
    model.save(f'./logs_f/models/MDPO_E_RE3_{random_seed}_{env_id}')
    end = time.localtime()

    f = open("outputs.txt", "a")
    f.write(f"model MDP_E_RE3 with seed = {random_seed} on {env_id} finished in start time_{start} and end time_{end} mean_reward {mean_reward} std_reward {std_reward} \n")
    f.close()
