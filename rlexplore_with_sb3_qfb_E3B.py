...
from minigrid.wrappers import ObservationWrapper, ReseedWrapper, FlatObsWrapper, ImgObsWrapper, FullyObsWrapper
...
from rllte.xplore.reward import NGU, E3B, RE3
...
def main():
    ...
    save_dir = f'./logs_f/QEB_envs/E_MDPO/E3B/{random_seed}/'
    ...
    callback_chkpt = CheckpointCallback(save_freq=1000, save_path=save_dir, name_prefix='E3B-MDPO')
    eval_callback = EvaluationAndCheckpointCallback(eval_env, save_dir=save_dir)
    # ===================== build the reward ===================== #
    irs = E3B(envs=envs, device=device, batch_size=64, latent_dim=128, lr=1e-3, update_proportion=0.5)
    # ===================== build the reward ===================== #

    start = time.localtime()
    model = MDPO("MlpPolicy", envs, learning_rate=2.5e-4, batch_size=batch, n_steps=128, ent_coef=0.00001, sgd_steps=10, verbose=1, seed=random_seed, device=device, tensorboard_log=f'./logs_f/{env_id}/MDPO_E/NGU/{random_seed}')
    model.learn(total_timesteps=total_timesteps, callback=[eval_callback, callback_chkpt, RLeXploreWithOnPolicyRL(irs)])
    
    mean_reward, std_reward = evaluate_policy(model, envs, n_eval_episodes=10)
    
    model.save(f'./logs_f/models/MDPO_E_E3B_{random_seed}_{env_id}')
    end = time.localtime()

    f = open("outputs.txt", "a")
    f.write(f"model MDP_E_E3B with seed = {random_seed} on {env_id} finished in start time_{start} and end time_{end} mean_reward {mean_reward} std_reward {std_reward} \n")
    f.close()
